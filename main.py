...
def analyze_audio(file_path):
    ...
    return int(overall_final_score),json.dumps(audio_detail, indent=4)
...
def analyze_audio_video():
    try: 
        ...
        if not file.filename.endswith(".mp4"):
            return jsonify({"message": "This is not an MP4 file", "status": False}), 400
        ...
        audio_path = "./myaud/audio_output.wav"
        ...
        try:
            vid_val = video_score["overall_video_score"]
        except:
            vid_val = video_score

       
        if isinstance(vid_val, (list, tuple)):
            vid_val = sum(vid_val) / len(vid_val)
        if isinstance(audio_score, (list, tuple)):
            audio_score = sum(audio_score) / len(audio_score)

        final_score = (vid_val + audio_score) / 2
        ...
        print(f"Error: {e}")
        return jsonify({"message": f"An error occurred during processing: {str(e)}", "status": False}), 500
